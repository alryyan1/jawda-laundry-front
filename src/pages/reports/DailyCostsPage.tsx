...
const DailyCostsPage: React.FC = () => {
  ...
  const {
    ...
    isLoading,
    isFetching,
  } = useQuery<DailyCostsReport, Error>({
    queryKey,
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-6">
        <StatCard
          ...
          icon={TrendingDown}
          value={
            report?.summary.total_cost !== undefined
              ? formatCurrency(
        ...
        <StatCard
          ...
          icon={CalendarDays}
          value={
            report?.summary.average_daily_cost !== undefined
              ? formatCurrency(
                  report.summary.average_daily_cost,
                  currencyCode,
                  i18n.language,
                )
              : undefined
          ...
          {isLoading ? (
            ...
            <ResponsiveContainer width="100%" height={350}>
              <BarChart data={report?.daily_data}>
                ...
                <YAxis
                  tickFormatter={(val) =>
                    formatCurrency(val, currencyCode, i18n.language).replace(
                      /\.00$/,
                      "",
                    )
                  }
            ...
            <TableBody>
              {isLoading
                ...
                : report?.daily_data.map((day) => (
                    ...
                    >
                      <TableCell
                        className={`font-medium ${day.daily_cost > 0 ? "font-bold text-lg text-red-600" : ""}`}
                      >
                        {format(parseISO(day.date), "EEEE, dd/MM/yyyy")}
                      </TableCell>
                      <TableCell
                        className={`text-center ${day.daily_cost > 0 ? "font-bold text-lg text-red-600" : ""}`}
                      >
                        {day.expense_count}
                      </TableCell>
                      <TableCell
                        className={`text-right font-mono ${day.daily_cost > 0 ? "font-bold text-lg text-red-600" : ""}`}
                      >
                        {formatCurrency(
                          day.daily_cost,
                          currencyCode,
