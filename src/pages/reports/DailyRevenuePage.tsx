    ...
    <div className="max-w-7xl mx-auto p-4 space-y-6">
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <StatCard
          ...
          value={
            report?.summary.average_daily_revenue !== undefined
              ? formatCurrency(
                  report.summary.average_daily_revenue,
                  currency,
                  i18n.language,
                )
              : undefined
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle>
              {t("revenueTrendFor", {
                month: report?.report_details.month_name || "...",
              })}
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <ResponsiveContainer width="100%" height={350}>
                <LineChart data={report?.daily_data}>
                  ...
                  <YAxis
                    yAxisId="left"
                    tickFormatter={(val) =>
                      formatCurrency(val, currency, i18n.language).replace(
                        /\.00$/,
                        "",
                      )
                    }
                  ...
                  <Tooltip
                    formatter={(value, name) =>
                      name === "Revenue"
                        ? formatCurrency(Number(value), currency, i18n.language)
                        : value
                    }
                  />
                  <Legend />
                ...
                <TableBody>
                  {isLoading
                    ...
                    : report?.daily_data.map((day) => (
                        ...
                        >
                          <TableCell
                            className={`font-medium ${day.daily_revenue > 0 ? "text-lg font-bold text-green-700 dark:text-green-300" : ""}`}
                          >
                            {format(parseISO(day.date), "EEEE, dd/MM/yyyy")}
                          </TableCell>
                          <TableCell
                            className={`text-center ${day.daily_revenue > 0 ? "text-lg font-bold text-green-700 dark:text-green-300" : ""}`}
                          >
                            {day.order_count}
                          </TableCell>
                          <TableCell
                            className={`text-right font-mono ${day.daily_revenue > 0 ? "text-lg font-bold text-green-700 dark:text-green-300" : ""}`}
                          >
                            {formatCurrency(
                              day.daily_revenue,
                              currency,
              ...
              {isLoading
                ...
                : report?.daily_data.map((day) => (
                    ...
                    >
                      <CardContent className="p-4">
                        <div
                          className={`font-medium ${day.daily_revenue > 0 ? "text-green-700 dark:text-green-300" : ""}`}
                        >
                          {format(parseISO(day.date), "EEEE, dd/MM/yyyy")}
                        </div>
                        <div className="flex justify-between items-center mt-2">
                          <span
                            className={`text-sm ${day.daily_revenue > 0 ? "text-green-700 dark:text-green-300" : "text-muted-foreground"}`}
                          >
                            {t("orderCount")}: {day.order_count}
                          </span>
                          <span
                            className={`font-mono font-bold ${day.daily_revenue > 0 ? "text-green-700 dark:text-green-300" : ""}`}
                          >
                            {formatCurrency(
                              day.daily_revenue,
                              currency,
