        ...
        <div className="max-w-7xl mx-auto p-4 space-y-6">
            ...
            <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                <StatCard 
                    ...
                    value={report?.summary.total_revenue !== undefined ? formatCurrency(report.summary.total_revenue, currency, i18n.language) : undefined} 
                    isLoading={isLoading} 
                    icon={DollarSign} 
                />
                <StatCard 
                ...
                <StatCard 
                    ...
                    value={report?.summary.average_daily_revenue !== undefined ? formatCurrency(report.summary.average_daily_revenue, currency, i18n.language) : undefined} 
                    isLoading={isLoading} 
                    icon={TrendingUp} 
                />
            </div>
                        ...
                        {isLoading ? (
                            ...
                            <ResponsiveContainer width="100%" height={350}>
                                <LineChart data={report?.daily_data}>
                                    ...
                                    <YAxis 
                                        yAxisId="left" 
                                        tickFormatter={(val) => formatCurrency(val, currency, i18n.language).replace(/\.00$/, '')} 
                                        fontSize={12} 
                                    />
                                    <YAxis 
                                        yAxisId="right" 
                                    ...
                                    />
                                    <Tooltip 
                                        formatter={(value, name) => name === 'Revenue' ? formatCurrency(Number(value), currency, i18n.language) : value} 
                                    />
                                    <Legend />
                                    <Line 
                                        yAxisId="left" 
                        ...
                        <div className="md:hidden space-y-3 p-4">
                            {isLoading ? (
                                ...
                                report?.daily_data.map(day => (
                                    <Card 
                                        key={day.date}
                                        className={day.daily_revenue > 0 ? 'border-l-4 border-l-green-500 bg-green-50 dark:bg-green-950/20' : ''}
                                    >
                                        <CardContent className="p-4">
                                            ...
                                            <div className="flex justify-between items-center mt-2">
                                                ...
                                                <span className={`font-mono font-bold ${day.daily_revenue > 0 ? 'text-green-700 dark:text-green-300' : ''}`}>
                                                    {formatCurrency(day.daily_revenue, currency, i18n.language)}
                                                </span>
                                            </div>
                                        </CardContent>
