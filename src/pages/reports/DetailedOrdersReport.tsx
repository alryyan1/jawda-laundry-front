...
const DetailedOrdersReport: React.FC = () => {
  ...
  const exportMutation = useMutation<Blob, Error, typeof filters>({
    mutationFn: (currentFilters) =>
      downloadOrdersReport({
        ...
        dateFrom: currentFilters.dateRange?.from
          ? format(currentFilters.dateRange.from, "yyyy-MM-dd")
          : undefined,
        dateTo: currentFilters.dateRange?.to
          ? format(currentFilters.dateRange.to, "yyyy-MM-dd")
          : undefined,
      }),
    onSuccess: (data) => {
      ...
      link.setAttribute(
        "download",
        `orders_report_${format(new Date(), "yyyy-MM-dd")}.csv`,
      );
    ...
    onError: (error) => {
      toast.error(error.message || t("reportExportFailed"));
    },
  });
            ...
            {isLoading ? (
              ...
              orders.map((order) => (
                <TableRow key={order.id}>
                  ...
                  <TableCell>
                    {formatDate(order.order_date, "dd/MM/yyyy", i18n.language)}
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right font-mono text-green-600">
                    {formatCurrency(order.paid_amount, "USD", i18n.language, 3)}
                  </TableCell>
