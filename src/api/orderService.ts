...
export const getOrders = async (
  ...
  if (filters?.category_sequence_search)
    params.category_sequence_search = filters.category_sequence_search;
  if (filters?.show_only_incomplete)
    params.show_only_incomplete = filters.show_only_incomplete;
...
export const createOrder = async (
  orderData: FrontendNewOrderFormData,
  serviceOfferingsList: ServiceOffering[],
): Promise<OrderResponseWithWarnings> => {
  console.log("Creating order with data:", orderData);
  console.log("Available service offerings:", serviceOfferingsList);

  const itemsPayload: BackendOrderItemPayload[] = orderData.items.map(
    (item) => {
  ...
  const payload: BackendOrderPayload = {
    ...
    due_date:
      orderData.due_date && orderData.due_date.trim() !== ""
        ? orderData.due_date
        : null,
 ...
 */
export const getOrderItemQuote = async (
  ...
  const { data } = await apiClient.post<QuoteItemResponse>(
    "/orders/quote-item",
    quoteApiPayload,
  );
  return data;
...
// src/api/orderService.ts
// ...
interface BackendOrderUpdatePayload {
  // More specific for update
...
const prepareOrderItemsPayload = (
  ...
  return items.map((item) => {
    ...
    const offeringId =
      item._derivedServiceOffering?.id ||
    ...
    if (!offeringId) {
      throw new Error(
        `Service offering could not be determined for an item. ProductType ID: ${item.product_type_id}, ServiceAction ID: ${item.service_action_id}`,
      );
    }
    ...
    return {
      ...
      product_description_custom: item.product_description_custom || null,
      length_meters:
        item.length_meters && String(item.length_meters).trim() !== ""
          ? parseFloat(String(item.length_meters))
          : null,
      width_meters:
        item.width_meters && String(item.width_meters).trim() !== ""
          ? parseFloat(String(item.width_meters))
          : null,
      notes: item.notes || null,
...
export const updateOrder = async (
  ...
  const itemsPayload = prepareOrderItemsPayload(
    orderData.items,
    allServiceOfferings,
  );

  const payload: BackendOrderPayload = {
    // BackendOrderPayload should allow optional fields for update
...
export const updateOrderDetails = async (
  orderId: string | number,
  payload: OrderDetailsUpdatePayload,
): Promise<Order> => {
  const { data } = await apiClient.patch<{ data: Order }>(
    `/orders/${orderId}`,
    payload,
  );
  return data.data;
...
export const sendOrderWhatsAppMessage = async (
  ...
  const { data } = await apiClient.post<{ message: string }>(
    `/orders/${orderId}/send-whatsapp-message`,
    {
      message,
    },
  );
...
export const sendOrderWhatsAppInvoice = async (
  ...
  const { data } = await apiClient.post<{ message: string }>(
    `/orders/${orderId}/send-whatsapp-invoice`,
  );
  return data;
};
...
export const updateOrderItemPickedUpQuantity = async (
  ...
  const { data } = await apiClient.patch<{ order_item: OrderItem }>(
    `/order-items/${orderItemId}/picked-up-quantity`,
    {
...
export const deleteOrderItem = async (
  ...
  const { data } = await apiClient.delete<{ order: Order; message: string }>(
...
export const cancelOrder = async (
  ...
  const { data } = await apiClient.post<{ order: Order; message: string }>(
...
export const updateOrderItemDimensions = async (
  ...
  const { data } = await apiClient.patch<{
    ...
    order_total: number;
    message: string;
  }>(`/order-items/${orderItemId}/dimensions`, dimensions);
  return data;
};
...
export const updateOrderItemQuantity = async (
  ...
  const { data } = await apiClient.patch<{
    ...
    order_total: number;
    message: string;
  }>(`/order-items/${orderItemId}/quantity`, { quantity });
  return data;
};
...
export const downloadOrdersListExcel = async (filters: {
  ...
  try {
    ...
    if (filters.productTypeId)
      params.append("product_type_id", filters.productTypeId);
    if (filters.dateFrom) params.append("date_from", filters.dateFrom);
    if (filters.dateTo) params.append("date_to", filters.dateTo);
    if (filters.category_sequence_search)
      params.append(
        "category_sequence_search",
        filters.category_sequence_search,
      );

    const response = await apiClient.get(
      `/reports/orders/export-csv?${params.toString()}`,
      {
        responseType: "blob",
      },
    );
