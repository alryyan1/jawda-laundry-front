...
export const POSNewOrderButton: React.FC = () => {
  ...
  const { shouldCreateNewOrder, resetCreateNewOrderTrigger } = useNewOrder();

  // Listen for global trigger (Spacebar from MainLayout)
  React.useEffect(() => {
    if (shouldCreateNewOrder && !createEmptyOrderMutation.isPending) {
      handleCreateOrder();
